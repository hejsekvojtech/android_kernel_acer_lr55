...
static int set_bssid_filter(struct wpa_supplicant *wpa_s, char *val)
{
	...
	while (pos) {
		...
		if (*pos == 'x' && *(pos+1) == 'x'){ //Disable bssid_filter
			os_free(wpa_s->bssid_filter);
			wpa_s->bssid_filter = NULL;
			wpa_s->bssid_filter_count = 0;
			return 0;
		}
					 ...
					 char *cmd, char *buf, size_t buflen)
{
	...
	} else if (os_strcasecmp(cmd, "auto_interworking") == 0) {
		res = os_snprintf(buf, buflen, "%d",
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
...
#ifdef CONFIG_MTK_HS20R1
	ret = os_snprintf(pos, end - pos,
		  "network id / ssid / bssid / flags / priority\n");
#else
	ret = os_snprintf(pos, end - pos,
	...
	while (ssid) {
		...
		pos += ret;
#ifdef CONFIG_MTK_HS20R1
		ret = os_snprintf(pos, end - pos, "\t%s%s%s%s\t%d",
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strcmp(cmd, "temp") == 0) {
		wpa_printf(MSG_DEBUG, "CTRL_IFACE: REMOVE_NETWORK TEMP");
		ssid = wpa_s->conf->ssid;
		while (ssid && ssid->temporary) {
			struct wpa_ssid *remove_ssid = ssid;
			id = ssid->id;
			ssid = ssid->next;
			//wpas_notify_network_removed(wpa_s, remove_ssid);
						...
						char *buf, size_t buflen)
{
...
#ifdef CONFIG_MTK_HS20R1
	ret = os_snprintf(pos, end - pos,
		"cred id / realm / username / domain / imsi / priority\n");
#else
	ret = os_snprintf(pos, end - pos,
	...
	while (cred) {
#ifdef CONFIG_MTK_HS20R1
	ret = os_snprintf(pos, end - pos, "%d\t%s\t%s\t%s\t%s\t%d\n",
...
{
//	ret += wpa_supplicant_get_sta_link_detect(wpa_s, buf + ret, buflen - ret);
	int link_len;

	link_len = wpa_drv_driver_cmd(wpa_s, "GET_STA_LINK_DETECT", buf + ret, buflen - ret);
	if (ret > 0) {
		ret += link_len;
	}
}
	...
	if(os_strncasecmp(cmd, "BEAMPLUS_", os_strlen("BEAMPLUS_"))==0) {
    	...
    	if (p2p != NULL) {
	    	if(os_strncasecmp(cmd, "BEAMPLUS_GO_RESERVE_START", os_strlen("BEAMPLUS_GO_RESERVE_START"))==0) {
		    	...
		    	wpas_driver_set_beamplus(wpa_s, start);
	    	}
	    	ret = 0;
    	}else{
...
#endif
#ifdef CONFIG_MTK_HS20R1
	} else if (os_strncmp(buf, "HS20_STA_EN", 11) == 0) {
		reply_len = hs20_sta_enable(wpa_s, buf + 11, reply, reply_size);
	} else if (os_strncmp(buf, "HS20_STA_DIS", 12) == 0) {
		reply_len = hs20_sta_disable(wpa_s, buf + 12, reply, reply_size);
		
#ifdef CONFIG_MTK_HS20R1_SIGMA
	} else if (os_strncmp(buf, "ADD_SCAN_INFO ", 14) == 0) {
		reply_len = sigma_hs20_add_scan_info(wpa_s, buf + 14, reply, reply_size);
	} else if (os_strncmp(buf, "GET_CURR_ASSOC", 14) == 0) {
		reply_len = sigma_get_current_assoc(wpa_s, buf + 14, reply, reply_size);
#if 0	// Replace by set bssid_filter
	} else if (os_strncmp(buf, "SET_BSSID_POOL ", 15) == 0) {
		reply_len = sigma_hs20_set_bssid_pool(wpa_s, buf + 15, reply, reply_size);
#endif
