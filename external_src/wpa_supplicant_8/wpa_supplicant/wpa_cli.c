...
static int wpa_cli_cmd_hs20_sta_en(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
	char cmd[4096];
	int res;
...
static int wpa_cli_cmd_hs20_sta_dis(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
	char cmd[4096];
	int res;
...
static int sigma_cmd_hs20_add_scan_info(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
	char cmd[4096];
	int res;
	...
	if (argc < 1 || argc > 3) {
		printf("Invalid ADD_SCAN_INFO command: needs at least one "
			   "argument\n");
		return -1;
	}
...
static int sigma_cmd_hs20_set_bssid_pool(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
    char cmd[4096];
    int res;
	...
	if(argc == 1){
		res = os_snprintf(cmd, sizeof(cmd), "SET_BSSID_POOL %s", argv[0]);
	}
	else if(argc == 2){
		res = os_snprintf(cmd, sizeof(cmd), "SET_BSSID_POOL %s %s", argv[0], argv[1]);
	}
	else {
		printf("Invalid set_bssid_pool command: needs one or two "
			   "arguments <0/1> <bssid pool>\n");
		return -1;
	}
...
static int sigma_cmd_get_curr_assoc(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
	return wpa_ctrl_command(ctrl, "GET_CURR_ASSOC");
}
...
#endif
#ifdef CONFIG_MTK_HS20R1
	{ "hs20_sta_en", wpa_cli_cmd_hs20_sta_en, NULL,
	  cli_cmd_flag_none,
	  "= Enable HS20 (including interworking)" },
  	{ "hs20_sta_dis", wpa_cli_cmd_hs20_sta_dis, NULL,
	  cli_cmd_flag_none,
	  "= Disable HS20 (excluding interworking)" },

#ifdef CONFIG_MTK_HS20R1_SIGMA
	{ "sta_add_scan_info", sigma_cmd_hs20_add_scan_info, NULL,
	  cli_cmd_flag_none,
	  "<-hHESSID> <-aACCS_NET_TYPE>= add scan info" },
    { "set_bssid_pool", sigma_cmd_hs20_set_bssid_pool, NULL,
      cli_cmd_flag_none,
      "<0/1> <bssid pool> = 0/1 for disable/enable bssid pool filter\n"
      "  bssid pool is bssids distinguished by ','" },    
    { "get_curr_assoc", sigma_cmd_get_curr_assoc, NULL,
      cli_cmd_flag_none,
      "= Get current association SSID & BSSID" },
#endif /* CONFIG_MTK_HS20R1_SIGMA */
#endif /*CONFIG_MTK_HS20R1 */

#if CONFIG_MTK_P2P /* [ALPS00618361] [WFD Quality Enhancement] */
