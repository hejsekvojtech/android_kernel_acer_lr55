...
#else /* PCSC_FUNCS */
#if CONFIG_RILD_FUNCS && CONFIG_MTK_TC1_FEATURE
/* read imsi from property and build NAI, don't use the one from UI. this feature request from LGE */
	char IMSI[16]="", PLMN[8];
	if (conf->eap_methods == NULL){
		wpa_printf(MSG_DEBUG, "EAP: no eap methods set, don't require NAI");
		return -1;
	}
	if(!property_get("gsm.sim.operator.imsi", IMSI, NULL)) {
		wpa_printf(MSG_ERROR,"EAP: gsm.sim.operator.imsi is not set yet");
		return -1;
	}
	if(property_get("gsm.sim.operator.numeric", PLMN, NULL)==5) {
		PLMN[7] = 0;
		PLMN[6] = PLMN[4];
		PLMN[5] = PLMN[3];
		PLMN[4] = '0';
	...
	else if(property_get("gsm.sim.operator.numeric", PLMN, NULL)==6) {
		PLMN[7] = 0;
		PLMN[6] = PLMN[5];
		PLMN[5] = PLMN[4];
		PLMN[4] = PLMN[3];
	...
	if(conf->imsi) {
		sprintf(conf->imsi,"%d%s@wlan.mnc%s.mcc%s.3gppnetwork.org",
			conf->eap_methods[0].method == EAP_TYPE_SIM,
			IMSI,PLMN+4,PLMN);
		conf->imsi_len = os_strlen(conf->imsi);
		wpa_printf(MSG_DEBUG, "EAP: NAI is %s", conf->imsi);
		return 0;
	}
#endif
	return -1;
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	if (sm->m && sm->m->get_identity &&
		...
		wpa_hexdump_ascii(MSG_DEBUG, "EAP: using anonymous identity",
			...
			eap_sm_get_scard_identity(sm, config) == 0 
#else
			config->imsi && os_strcmp((const char*)config->imsi, "\"none\"") != 0
#endif
               && (eap_allowed_method(sm, EAP_VENDOR_IETF, EAP_TYPE_SIM)
...
const u8 * eap_get_config_identity(struct eap_sm *sm, size_t *len)
{
	...
	if (config == NULL)
		return NULL;
#ifdef CONFIG_RILD_FUNCS     
    if (config->imsi && os_strcmp((const char*)config->imsi,"\"none\"") != 0
         && (eap_allowed_method(sm, EAP_VENDOR_IETF, EAP_TYPE_SIM)
              || eap_allowed_method(sm, EAP_VENDOR_IETF, EAP_TYPE_AKA))){
		*len = config->imsi_len;
		return config->imsi;
