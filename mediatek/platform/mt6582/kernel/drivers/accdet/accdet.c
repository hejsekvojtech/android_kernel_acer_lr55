...
/****************************************************************/
/***function, for write int value to device attribute                                                                     **/
/****************************************************************/
static int write_int(char * paths, int value)
{
	struct file  *vibrate_file;
	mm_segment_t fs;	
	ssize_t ret;
    char vibrate_time[10];
	int len;
	
	len = sprintf(vibrate_time,"%d",value);

	vibrate_file = filp_open(paths, O_WRONLY, 0666);	
	if(IS_ERR(vibrate_file))
	{
        printk("%s: filp_open for write error!\n", __func__);
		return -ENOENT;
	}
	else
	{
		...
		ret = vibrate_file->f_op->write(vibrate_file,vibrate_time,len,&vibrate_file->f_pos);
		if(ret != len)
		{
			printk("%s: write error!\n", __func__);
			filp_close(vibrate_file,NULL);
			return -EIO;
		}
...
/****************************************************************/
/***function, vibrate when headset plug in for customer k-touch need                                                                    **/
/****************************************************************/
static accdet_plugin_vibrate(void)
{
   int value = 120; // vibrate on sustain time (ms)
   
	write_int("/sys/class/timed_output/vibrator/vibr_on_sustain", value);
}
