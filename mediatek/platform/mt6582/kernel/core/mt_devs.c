...
/*[---Start add by Rachel, add hwid, 20140428---]*/
#include <linux/proc_fs.h>
/*[---End add by Rachel, add hwid, 20140428---]*/

int cci_bootreason = 0;
int cci_bootreason_read(char *page, char **start, off_t off, int count, int *eof, void *data);
...
static int __init cci_bootreason_from_cmdline(char *bootreason)
{
    sscanf(bootreason, "%d", &cci_bootreason);
        printk(KERN_ERR "Read CCI BOOTREASON =%d from cmdline!!\n",cci_bootreason);
 
        return 1;
}
__setup("boot_reason=", cci_bootreason_from_cmdline);

int cci_bootreason_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        int len = 0;
        len += sprintf(page, "%d", cci_bootreason);
        return len;
}
...
static ssize_t fp_show(char *buf)
{
    char fp[FINGERPRINT_LEN] = {'\xbe', '\x29', '\x75', '\x42', '\xcf', '\x08', '\x35', '\x66'};
    strncpy(buf, fp, FINGERPRINT_LEN);
    return FINGERPRINT_LEN;
}

struct sysinfo_attribute fp_attr = {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36))
    .attr = {"fp", THIS_MODULE, 0444},
#else
    .attr = {"fp", 0644},
#endif
    .show = fp_show,
...
/*[---Start add by Rachel, add hall sensor driver, 20130904---]*/
struct platform_device sensor_hallsensor = {
	.name	 = "hall_gpio",
	.id            = -1,
};
/*[---End add by Rachel, add hall sensor driver, 20130904---]*/
...
static char * hw_rev_str[] = {"EVT", "DVT1", "DVT2", "PVT", "MP"};

int __init parse_tag_hwid(const struct tag *tags)
{
	int hwid = 0, find = 0, bandid = 0;
...
static char *hw_band_string[] = {"EU", "US"} ;
static int modem_band_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0 ;
    int band = -1 ;

    band = mt_get_gpio_in_base(GPIO93) ;
...
{
    int i = 0, retval = 0;
          create_proc_read_entry("CCI_BOOT_REASON", 0, NULL, cci_bootreason_read, NULL);
...

	{
		...
		key = 0;
#endif
		if ((key != 0) && ((g_boot_mode == NORMAL_BOOT) || (g_boot_mode==ALARM_BOOT))) 
...
/*[---Start add by Rachel, add hall sensor driver, 20130904---]*/
#if defined(CUSTOM_KERNEL_HALLSENSOR)
...
//#endif
	...
	create_proc_read_entry("band", 0444, NULL, modem_band_proc_read, NULL);
