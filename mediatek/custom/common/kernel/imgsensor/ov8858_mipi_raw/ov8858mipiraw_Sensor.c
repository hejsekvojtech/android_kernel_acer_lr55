...
/*end liujie for sensor id file add*/



//===========add by xushengwei==============//
static bool ov8858_otp_flag=0 ;
...
/*begin liujie for  8858 add*/
static int G_customer_id = 0;		//区分不同模组的变量
static int G_lens_id = 0;      
static int G_vcam_id = 0; 
/*end liujie for  8858 add*/
...
#define OTP_BUF_AWB_OFFSET 	0x10//modify by xushengwei//0x20
#define OTP_AWB_BASE_ADDR OTP_BUF_ADDR+OTP_BUF_AWB_OFFSET //0x7020

#define OTP_BUF_VCM_OFFSET 	0x20//modify by xushengwei//0x30
#define OTP_VCM_BASE_ADDR OTP_BUF_ADDR+OTP_BUF_VCM_OFFSET//0x7030

#define OTP_BUF_LCS_OFFSET 	0x2A//modify by xushengwei//0x3A
...
/*begin liujie for truly add*/
#define TRULY_RG_TYPICAL 0x014d//Truly AWB typical R/G
#define TRULY_BG_TYPICAL 0x0138//Truly AWB typical B/G
#define TRULY_MODEL 0x02
#define QUYEI_MODEL 0x05
/*end liujie for truly add*/

int RG_Ratio_Typical = 0x011D;//0x230;
int BG_Ratio_Typical = 0x012E;//0x2B1;


//xushengwei add
//#define RG_TYPICAL 0x011D//
//#define BG_TYPICAL 0x012E//

#define OTPLOG(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "OTP",  fmt, ##arg)

static struct otp_struct1 current_otp;
...
// For HW
// index: index of otp group. (1, 2, 3)
// otp_ptr: pointer of otp_struct1
// return: 	0, 
static int read_otp_info(int index, struct otp_struct1 * otp_ptr)
{
	int awb_light_lsb;
	...
	// disable otp read
	//OV8858_write_cmos_sensor(0x3d81, 0x00);
/*begin liujie for truly model add*/
	G_customer_id = (*otp_ptr).module_integrator_id;
	G_lens_id = (*otp_ptr).lens_id;
/*end liujie for truly model add*/
	clear_otp_buffer(start_addr,end_addr);
...
static int check_otp_wb()
{
	...
	for(index = 1;index<=3;index++)
	{
		...
		OTPLOG("check_otp_wb, wb_flag = 0x%x \n",flag);
...
// otp_ptr: pointer of otp_struct1
// return: 	0, 
static int read_otp_wb(int index, struct otp_struct1 * otp_ptr)
{
	int awb_light_lsb;
	int group_addr,start_addr,end_addr;
...
// otp_ptr: pointer of otp_struct1
// return: 	0, 
static int read_otp_lenc(int index, struct otp_struct1 * otp_ptr)
{
	int bank, i;
	int group_addr,start_addr,end_addr;
...
// otp_ptr: pointer of otp_struct1
static int update_lenc(struct otp_struct1 * otp_ptr)
{
	int i, temp;
...
static int update_otp_wb()
{
    ...
    OTPLOG("OV8858_Upate_Otp_WB, r/g:0x%x, b/g:0x%x\n", rg, bg);

	//calculate G gain
	//0x400 = 1x gain
/*begin liujie for truly model add*/
	if( G_customer_id == TRULY_MODEL)
	{
		RG_Ratio_Typical = TRULY_RG_TYPICAL;
		BG_Ratio_Typical = TRULY_BG_TYPICAL;
		OTPLOG("this is truly molden R/G is %d B/G is %d\n",RG_Ratio_Typical,BG_Ratio_Typical );
	}
/*end liujie for truly model add*/	
    ...
    switch (group_idx)
    {
        case PRE_GAIN:
           switch (item_idx)
          {
			...
			//temp_gain= (temp_para/OV8858.sensorBaseGain) * 1000;
   ...
   switch (group_idx)
    {
        case PRE_GAIN:
		 ...
		 temp_gain=((ItemValue*BASEGAIN+500)/1000);			//+500:get closed integer value
          ...
          {
//             temp_para=(temp_gain * OV8858.sensorBaseGain + BASEGAIN/2)/BASEGAIN;
          }
          else
			  ASSERT(0);
...
void OV8858PreviewSetting(void)
{
	...
	// ++++++++  ;lines per frame=1244(0x4dc)
		...
		OV8858_write_cmos_sensor( 0x3814, 0x03);  // ; x odd inc
		OV8858_write_cmos_sensor( 0x3821, 0x47);  // ; mirror on, bin on
		OV8858_write_cmos_sensor( 0x382a, 0x03);  // ; y odd inc
		OV8858_write_cmos_sensor( 0x3830, 0x08);  //
		OV8858_write_cmos_sensor( 0x3836, 0x02);  //
...
void OV8858CaptureSetting(void)
{
	...
	// +++++++	;lines per frame=2474(0x9aa)
		...
		#endif
		#if 1
		//;;PCLK=HTS*VTS*fps=0x978*0x9aa*24=2424*2474*24=144M
		OV8858_write_cmos_sensor( 0x380c, 0x09);  // ; HTS H
		OV8858_write_cmos_sensor( 0x380d, 0x78);  // ; HTS L
		#endif
		//OV8858_write_cmos_sensor( 0x380d, 0x98);  // ; HTS L
...
UINT32 OV8858Open(void)
{
	...
	#ifdef OV8858_OTP
//	if(ov8858_otp_flag==0)
//	{
		update_otp_info();	
		update_otp_wb();
		update_otp_lenc();
		ov8858_otp_flag =1;
		OV8858DB("OV8858DB OPEN ov8858_otp_flag=%d\n",ov8858_otp_flag);
//	}
	#endif
