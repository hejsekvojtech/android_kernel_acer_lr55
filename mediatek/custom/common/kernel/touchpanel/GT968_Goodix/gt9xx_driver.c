...
u8 pnl_init_error = 0;
#if GTP_HALL_CLOSE_CFG
u8 grp_cfg_hallclose_version = 0;
volatile u8 tp_wake_flag; 
#endif
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    {
		#if GTP_HALL_CLOSE_CFG
		if(mt_get_gpio_in(GPIO_MHALL_EINT_PIN))  //gpio = 1, hall far away, in wake up state
        	ret = gtp_i2c_write(client, config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
		else  //gpio = 0, hall close, in sleep state
			ret = gtp_i2c_write(client, config_hallclose, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
		#else
			ret = gtp_i2c_write(client, config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
		#endif
...
#if GTP_HALL_CLOSE_CFG
void forhall_gtp_send_cfg()
{
	s32 ret = 0;
	if(tp_wake_flag){
		msleep(300); //waiting for config to store in IC
		ret = gtp_send_cfg(i2c_client_point);
		if (ret < 0)
		{
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	#if GTP_HALL_CLOSE_CFG
	u8 *send_cfg_buf_hallclose[] = {cfg_info_group1_hallclose, cfg_info_group2, cfg_info_group3,
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            if (opr_buf[0] < 90)
            {
				...
				#if GTP_HALL_CLOSE_CFG
                grp_cfg_hallclose_version = send_cfg_buf_hallclose[sensor_id][0];       // backup group config version
                send_cfg_buf_hallclose[sensor_id][0] = 0x00;
            	#endif
                fixed_config = 0;
            }
    ...
    memcpy(&config_hallclose[GTP_ADDR_LENGTH], send_cfg_buf_hallclose[sensor_id], cfg_len);
	tp_wake_flag = 1;
	#endif
...
#endif
    {
        ...
        config[cfg_len] = (~check_sum) + 1;
		#if GTP_HALL_CLOSE_CFG
        config_hallclose[GTP_ADDR_LENGTH] = grp_cfg_hallclose_version;
        check_sum = 0;
        for (i = GTP_ADDR_LENGTH; i < cfg_len; i++)
        {
            check_sum += config_hallclose[i];
...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
    ...
    //get ref file data
    flp = filp_open(GTP_BAK_REF_PATH, O_RDWR | O_CREAT, FILE_MODE);
    if (IS_ERR(flp))
    {
...
static u8 gtp_main_clk_proc(struct i2c_client *client)
{
    ...
    else
    {
        ...
        flp = filp_open(GTP_MAIN_CLK_PATH, O_RDWR | O_CREAT, FILE_MODE);
        if (!IS_ERR(flp))
        {
            flp->f_op->llseek(flp, 0, SEEK_SET);
...
static DEVICE_ATTR(touch_firmware_version, S_IRUGO|S_IWUSR, touch_firmware_version_show, NULL);
static DEVICE_ATTR(touch_firmware_version_check, S_IRUGO|S_IWUSR, touch_firmware_version_check_show, NULL);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    gt91xx_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, FILE_MODE, NULL, &config_proc_ops);
...
//    mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	...
	ret = sysfs_create_file(&client->dev.kobj, &dev_attr_touch_firmware_version.attr);
	if(ret)
		GTP_ERROR("tpd_i2c_probe sysfs_create_file ret = %d\n", ret);

	ret = sysfs_create_file(&client->dev.kobj, &dev_attr_touch_firmware_version_check.attr);
	if(ret)
		GTP_ERROR("tpd_i2c_probe sysfs_create_file ret = %d\n", ret);
...
static int touch_event_handler(void *unused)
{
    ...
    #endif
	#ifdef TPD_HAVE_BUTTON  //convert key event to touch event
	#define GTP_MAX_KEY_NUM ( sizeof( tpd_keys_local )/sizeof( tpd_keys_local[0] ) )
        key_value = point_data[3 + 8 * touch_num];
        ...
        if (key_value || pre_key)
        {
            if (!pre_touch)
            {
                for (i = 0; i < GTP_MAX_KEY_NUM; i++)
                {
					if( key_value & (0x01 << i) )
					{
                    	tpd_down(tpd_keys_dim_local[i][0], tpd_keys_dim_local[i][1], tpd_keys_dim_local[0][2]/2, 0);
						break;
					}
					else if((!key_value) && pre_key)
					{
						tpd_up(0, 0, 0);
						break;
					}
...
//				input_sync(tpd->dev);
			...
			GTP_DEBUG("key_value:%02x, pre_key:%02x.", key_value, pre_key);
        }
    #endif
        pre_key = key_value;
