...
static ssize_t store_vibr_on_sustain(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
   int sustain_time;
	...
	{
		printk("[vibrator]buf is %s and size is %d \n",buf,size);
		if(1 == sscanf(buf, "%d", &sustain_time))
		{   
			printk("[vibrator]store_vibr_on_sustain: %d!!!\n",sustain_time);
			vibrator_enable(&mtk_vibrator, sustain_time);
		}
		else
		{
			printk("store_vibr_on_sustain: invalid format\n");
		}
	}
...
static DEVICE_ATTR(vibr_on_sustain, 0664, NULL, store_vibr_on_sustain);
...
{	
	...
	ret = device_create_file(mtk_vibrator.dev,&dev_attr_vibr_on_sustain);
    if(ret)
    {
        printk("[vibrator]device_create_file vibr_on_sustain fail! \n");
    }
