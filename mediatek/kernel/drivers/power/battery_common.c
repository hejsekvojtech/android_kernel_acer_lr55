...
/*[---Start add by Rachel, tun on/off charging, 20140514---]*/
extern  CHR_CURRENT_ENUM g_temp_CC_value;
/*[---End add by Rachel, tun on/off charging, 20140514---]*/
	...
	if(BMT_status.charger_exist == KAL_TRUE)
	{
		if(previous_charger_exist == KAL_FALSE)
		{
			...
			if (BMT_status.charger_type == STANDARD_CHARGER) {
				...
				if(BMT_status.bat_vol < BATTERY_VOL_THRES)
				{
					data = CHARGE_CURRENT_1000_00_MA / 100;
				}
				else
				{
					data = CHARGE_CURRENT_500_00_MA / 100;
				}
...
static void mt_battery_charger_detect_check(void)
{
    if( upmu_is_chr_det() == KAL_TRUE )
    {
    	...
    	if(blue_led_flag == KAL_FALSE)
    	{
    		blue_led_flag = KAL_TRUE;
...
static ssize_t store_disable_charger(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_xlog_printk(ANDROID_LOG_ERROR, "Power/Battery", "store_disable_charger-->disable_charger_value:%d\n",disable_charger_value);
	...
	if(buf != NULL && size != 0)
	{
	    if(disable_charger_value == 1) /*  Disable charger */
	    {
	        ...
	        battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);	
	        battery_xlog_printk(ANDROID_LOG_ERROR, "Power/Battery", "store_disable_charger-->pchr_turn_off_charging-->disable_charger_value:%d\n",disable_charger_value);
            }
            ...
            if(disable_charger_value == 0) /*  Enable charger */
            {
                ...
                battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
		  battery_xlog_printk(ANDROID_LOG_ERROR, "Power/Battery", "store_disable_charger-->pchr_turn_on_charging-->disable_charger_value:%d\n",disable_charger_value);
            }		
	}
	battery_xlog_printk(ANDROID_LOG_ERROR, "Power/Battery", "store_disable_charger-->end\n");
